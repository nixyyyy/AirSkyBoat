...
local m = Module:new("era_weaponskill_formulas")



-- Obtains alpha, used for working out WSC on legacy servers
local getAlpha = function(level)
...
local cRangedRatio = function(attacker, defender, params, ignoredDef, tp)
...
local getRangedHitRate = function(attacker, target, capHitRate, bonus, wsParams, calcParams)
    local accVarryTP = 0
...
local calculateRawWSDmg = function(attacker, target, wsID, tp, action, wsParams, calcParams, isRanged)
...
local doPhysicalWeaponskill = function(attacker, target, wsID, wsParams, tp, action, primaryMsg, taChar)
...
-- damage is returned, handles reductions based on target resistances and passes off to xi.weaponskills.takeWeaponskillDamage.
local doRangedWeaponskill = function(attacker, target, wsID, wsParams, tp, action, primaryMsg)
...
local doMagicWeaponskill = function(attacker, target, wsID, wsParams, tp, action, primaryMsg)
...
local takeWeaponskillDamage = function(defender, attacker, wsParams, primaryMsg, attack, wsResults, action)
...
local getMeleeDmg = function(attacker, weaponType, kick)
...
local getHitRate = function(attacker, target, capHitRate, bonus, isSubAttack, wsParams, calcParams)
    if isSubAttack == nil then
        isSubAttack = false
    end
...
local fTP = function(tp, ftp1, ftp2, ftp3)
    if (tp < 1000) then
        tp = 1000
    end
...
local calculatedIgnoredDef = function(tp, def, ignore1, ignore2, ignore3)
    if tp >= 1000 and tp < 2000 then
...
local cMeleeRatio = function(attacker, defender, params, ignoredDef, tp, slot)
    local flourisheffect = attacker:getStatusEffect(xi.effect.BUILDING_FLOURISH)
...
local handleBlock = function(attacker, target, finaldmg)
...
local handleParry = function(attacker, target, missChance, guaranteedHit)
...
local fSTR = function(atk_str, def_vit, weaponRank)
...
local fSTR2 = function(atk_str, def_vit, weaponRank)
    local dSTR = atk_str - def_vit
...
local generatePdif = function(cratiomin, cratiomax, melee)
...
local getStepAnimation = function(skill)
    if skill <= 1 then
        return 15
    elseif skill <= 3 then
        return 14
...
local getFlourishAnimation = function(skill)
    if skill <= 1 then
        return 25
    elseif skill <= 3 then
        return 24
...
local handleWSGorgetBelt = function(attacker)
    local ftpBonus = 0
    local accBonus = 0
...
m:addOverride("xi.weaponskills.getAlpha", getAlpha)
m:addOverride("xi.weaponskills.cRangedRatio", cRangedRatio)
m:addOverride("xi.weaponskills.getRangedHitRate", getRangedHitRate)
m:addOverride("xi.weaponskills.calculateRawWSDmg", calculateRawWSDmg)
m:addOverride("xi.weaponskills.doPhysicalWeaponskill", doPhysicalWeaponskill)
m:addOverride("xi.weaponskills.doRangedWeaponskill", doRangedWeaponskill)
m:addOverride("xi.weaponskills.doMagicWeaponskill", doMagicWeaponskill)
m:addOverride("xi.weaponskills.takeWeaponskillDamage", takeWeaponskillDamage)
m:addOverride("xi.weaponskills.getMeleeDmg", getMeleeDmg)
m:addOverride("xi.weaponskills.getHitRate", getHitRate)
m:addOverride("xi.weaponskills.fTP", fTP)
m:addOverride("xi.weaponskills.calculatedIgnoredDef", calculatedIgnoredDef)
m:addOverride("xi.weaponskills.cMeleeRatio", cMeleeRatio)
m:addOverride("xi.weaponskills.handleBlock", handleBlock)
m:addOverride("xi.weaponskills.handleParry", handleParry)
m:addOverride("xi.weaponskills.fSTR", fSTR)
m:addOverride("xi.weaponskills.fSTR2", fSTR2)
m:addOverride("xi.weaponskills.generatePdif", generatePdif)
m:addOverride("xi.weaponskills.getStepAnimation", getStepAnimation)
m:addOverride("xi.weaponskills.getFlourishAnimation", getFlourishAnimation)
m:addOverride("xi.weaponskills.handleWSGorgetBelt", handleWSGorgetBelt)
